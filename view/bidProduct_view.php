...
<body class="bg-gray-200">
    <!-- nav bar -->
        ...
        <div class="container mx-auto w-full lg:w-[80%] p-2 bg-white rounded-lg shadow-md my-5 py-5">
            <div class="flex flex-col lg:flex-row">
                <div class="lg:w-1/2 flex flex-col items-center">
                    <img class="w-[320px] h-auto rounded-lg" src="<?php echo htmlspecialchars($product['image']); ?>" alt="Product Image">
                ...
                <div class="lg:w-1/3 lg:pl-8 mt-0 lg:mt-0 ">
                    <h2 class="text-5xl font-semibold text-[#897062]"><?php echo htmlspecialchars($product['product_name']); ?></h2>
                    <p class="mt-2 text-[#746557]"><?php echo htmlspecialchars($product['description']); ?></p>
                    <div class="mt-2 flex items-center space-x-2">
                        <span class="text-xl font-semibold text-[#746557]">Rs.<?php echo htmlspecialchars($product['price']); ?></span>
                        <?php if ($auction['start_price'] > 0) { ?>
                            <span class="text-sm line-through text-[#746557]">Rs.<?php echo htmlspecialchars($auction['start_price']); ?></span>
                        <?php } ?>
                    </div>

                    <div class="mt-4  bg-[#CEC0B9] text-center rounded-md py-2 px-">
                        <p class="text-[22px] font-semibold text-[#746557]">Highest Bid: Rs.<span id="highestBid " class="text-[#897062] font-semibold "><?php echo $highest_bid; ?></span></p>
                    </div>
                    ...
                    <div class="mt-4">
                        <?php if (strtotime($auction['start_time']) > time()) { ?>
                            <p class="text-lg font-semibold text-[#746557]">Starts In: <span id="countdown"></span></p>
                            <script>
                                updateCountdown("<?php echo $auction['start_time']; ?>", "countdown");
                            </script>
                        <?php } else if (strtotime($auction['end_time']) >= time()) { ?>
                            <p class="text-lg font-semibold text-[#746557]">Ends In: <span id="countdown"></span></p>
                            <script>
                                updateCountdown("<?php echo $auction['end_time']; ?>", "countdown");
                            </script>
                        <?php } else { ?>
                            <p class="text-lg font-semibold text-[#746557]">Auction Finished</p>
                        <?php } ?>
                    </div>
                </div>
            </div>
